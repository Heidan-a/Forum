...
import {Refresh, Select, User,Message} from "@element-plus/icons-vue";
...
function beforeUploadAvatar(rawFile){
  if(rawFile.type !== 'image/jpeg' && rawFile.type !== 'image/png'){
    ElMessage.warning("只支持JPG和PNG格式")
  }else if(rawFile.size　> 1024 * 256){
    ElMessage.warning("图片大小不能超过256KB")
  }
}
...
<template>
  <div style="display: flex;max-width: 950px;margin: auto">
    <div class="setting-left">
      <card :icon="User" title="账号信息设置" desc="在这里编辑您的个人信息设置,您可以在隐私设置中选择是否展示这些信息">
        <el-form :model="baseForm" :rules="rules" ref="baseFormRef" label-position="top" style="margin: 0 10px 10px 10px" v-loading="loading.form">
      ...
      <card style="margin-top: 10px" :icon="Message" title="电子邮件设置" desc="您可以在这里修改你的电子邮件设置">
        <el-form :rules="rules" @validate="onValidate" :model="emailForm" ref="emailFormRef"  label-position="top">
          <el-form-item label="电子邮件" prop="email">
            <el-input v-model="store.user.email"></el-input>
          </el-form-item>
          <el-form-item prop="code">
            <el-row style="width: 100%" :gutter="10">
              <el-col :span="18">
                  <el-input placeholder="请先获取验证码" v-model="emailForm.code"></el-input>
              </el-col>
              <el-col :span="6">
                <el-button type="success" @click="sendEmailCode" :disabled="!isValidEmail || coldTime > 0" plain style="width: 100%">
                  {{ coldTime > 0 ? `请稍候等待${coldTime}秒` : '获取验证码' }}
                </el-button>
              </el-col>
            </el-row>
          </el-form-item>
    ...
    <div class="setting-right">
      <div style="position: sticky;top: 20px">
        ...
        <card style="margin-top: 10px;font-size: 16px" >
